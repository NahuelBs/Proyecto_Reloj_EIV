...
static digital_output_t ReserveDigitalOutput(void) {
  ...
  digital_output_t slot                         = NULL;     // puntero de retorno inicializado en NULL para
...
digital_output_t CreateDigitalOutput(uint8_t port, uint32_t pin, bool inverted) {
  digital_output_t self = ReserveDigitalOutput();     // reserva, si existe, un
                                                      // espacio disponible, un slot
...
void ActivateDigitalOutput(digital_output_t self) {
  ...
  Chip_GPIO_SetPinState(LPC_GPIO_PORT, self->port, self->pin,
                        !self->inverted);     // inverted se inicializ por defecto
                                              // en false , true -> high
}
...
void DeactivateDigitalOutput(digital_output_t self) {
  // funcion encargada de definir el nivel de voltaje inicial (alto o bajo)
  Chip_GPIO_SetPinState(LPC_GPIO_PORT, self->port, self->pin,
                        self->inverted);     // inverted se inicializ por defecto en
                                             // false , false -> low
}
