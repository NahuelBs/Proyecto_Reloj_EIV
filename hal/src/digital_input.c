...
digital_input_t create_digital_input(uint8_t port, uint32_t pin, bool inverted) {
  ...
  if (self) {
    ...
    gpio_set_dir(self->port, self->pin, false);     // false->entrada
  }
  return self;
}

bool get_state_digital_input(digital_input_t self) {
  return gpio_read(self->port, self->pin) != self->inverted;     // retorna el estado de la entrada false->presionado
                                                                 // (hal fabricante),
...
int has_changed_digital_input(digital_input_t self) {     // analiza si hubo algun cambio
  ...
  } else if (!current_state && self->last_state) {     // estado actual es falso (0) y el estado
